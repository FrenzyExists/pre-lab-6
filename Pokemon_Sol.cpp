...
public:
    ...
    {
        if (this->myMoves.size() >= 4)
        {
            return;
        }
        this->myMoves.push_back(newMove);
    }

    void addMove(string newName, pkmType newType, int newPower)
    {
        if (this->myMoves.size() >= 4)
        {
            return;
        ...
        myMoves.push_back(Move(newName, newType, newPower));
    }
...
class Charmander : public Pkm
{
public:
    Charmander(int level, pkmType firstType, pkmType secondType, string nickname, int hp) : Pkm(level, firstType, secondType, nickname, hp)
    {
        this->setGrowl("Char Char");
    }
};

void makePokemonSound(Pkm *pokemon)
{
    pokemon->growl();
}
